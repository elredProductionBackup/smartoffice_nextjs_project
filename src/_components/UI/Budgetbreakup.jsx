...
//             </div>
//           ) : (
//             <button onClick={handleAddVersion} className="flex items-center gap-2 px-3 py-1.5 border border-gray-300 text-gray-700 text-sm rounded-lg hover:bg-gray-50 transition-colors">
...
import { FaXTwitter } from "react-icons/fa6";
import { fmt, useEventCosting } from "../Eventcostingcontext";
...
export default function BudgetBreakup() {
  ...
  const isOverallOverBudget = totalApprovedSpend > latestGrandTotal && totalApprovedSpend > 0;
  ...
  return (
    <div className="border-t border-gray-200 pt-6 bg-white py-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-base font-semibold text-gray-900">Budget Breakup</h3>
          <p className="text-xs text-gray-500 mt-0.5">Same line items as Expense Items, with planned amounts by version.</p>
        </div>
        <button
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        {versions.map((v) => (
          <div
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${
              v.id === latestVersion.id ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300"
            }`}
          >
            <span>{v.label}</span>
            {versions.length > 1 && (
        ...
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border bg-white text-gray-700 border-gray-300">
          <span>Actual</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full text-sm border-collapse min-w-[900px]">
          <thead>
            <tr className="bg-gray-50">
              <th rowSpan={2} className="text-left py-3 px-4 font-semibold text-gray-700 border-b border-gray-200 align-bottom w-[220px]">
                Expense
              </th>
              {versions.map((v) => (
                <th key={v.id} colSpan={3} className="text-center py-2 px-2 font-semibold text-gray-700 border-b border-l border-gray-200">
                  {v.label}
                </th>
              ))}
              <th colSpan={2} className="text-center py-2 px-2 font-semibold text-gray-700 border-b border-l border-gray-200">
                Actual
              </th>
            </tr>
            <tr className="bg-gray-50 text-xs text-gray-500">
              {versions.map((v) => (
                <Fragment key={v.id + "-sub"}>
                  <th className="py-2 px-2 text-right border-b border-l border-gray-200 font-medium w-16">Qty</th>
                  <th className="py-2 px-2 text-right border-b border-gray-200 font-medium w-24">Unit cost</th>
                  <th className="py-2 px-2 text-right border-b border-gray-200 font-medium w-24">Total</th>
                </Fragment>
              ))}
              <th className="py-2 px-2 text-right border-b border-l border-gray-200 font-medium w-16">Status</th>
              <th className="py-2 px-3 text-right border-b border-gray-200 font-medium w-28">Total</th>
            </tr>
          </thead>
          <tbody>
            {sections.map((section) => {
              const sectionExpenses = expenses.filter((e) => categoryToSectionId(e.category) === section.id);
              if (sectionExpenses.length === 0) return null;

              return (
                <Fragment key={section.id}>
                  <tr className="bg-gray-100">
                    <td colSpan={3 * versions.length + 3} className="py-2 px-4 font-semibold text-gray-700 border-b border-gray-200 text-xs uppercase tracking-wide">
                      {section.name}
                    </td>
                  </tr>
                  {sectionExpenses.map((exp) => {
                    const actualTotal = exp.subItems.reduce((s, si) => s + (Number(si.amount) || 0), 0);
                    const isApproved = exp.approvalStatus === "Approved";
                    const latestPlan = getPlan(exp, latestVersion.id);
                    const latestPlanTotal = (Number(latestPlan.qty) || 0) * (Number(latestPlan.unitCost) || 0);
                    const over = isApproved && actualTotal > latestPlanTotal && latestPlanTotal > 0;

                    return (
                      <tr key={exp.id} className={`transition-colors ${isApproved ? "bg-green-50" : "hover:bg-gray-50"}`}>
                        <td className="py-2 px-4 text-gray-700 border-b border-gray-100">
                          <div className="font-medium text-gray-800">{exp.category}</div>
                          {exp.description && <div className="text-xs text-gray-400">{exp.description}</div>}
                        </td>
                        {versions.map((v) => {
                          const plan = getPlan(exp, v.id);
                          const total = (Number(plan.qty) || 0) * (Number(plan.unitCost) || 0);
                          return (
                            <Fragment key={v.id + exp.id}>
                              <td className="py-2 px-2 border-b border-l border-gray-100">
                                <EditableCell value={plan.qty} onChange={(val) => handlePlanChange(exp.id, v.id, "qty", val)} />
                              </td>
                              <td className="py-2 px-2 border-b border-gray-100">
                                <EditableCell value={plan.unitCost} onChange={(val) => handlePlanChange(exp.id, v.id, "unitCost", val)} />
                              </td>
                              <td className="py-2 px-3 text-right border-b border-gray-100 font-medium text-gray-700">{fmt(total)}</td>
                            </Fragment>
                          );
                        })}
                        <td className="py-2 px-2 border-b border-l border-gray-100 text-center">
                          <span className={`inline-block w-2 h-2 rounded-full ${STATUS_DOT[exp.approvalStatus]}`} title={exp.approvalStatus} />
                        </td>
                        <td className={`py-2 px-3 text-right border-b border-gray-100 font-medium ${over ? "text-red-600" : isApproved ? "text-green-700" : "text-gray-600"}`}>
                          <div className="flex items-center justify-end gap-1">
                            {over && <FiAlertTriangle className="w-3 h-3 text-red-500" />}
                            {fmt(actualTotal)}
                          </div>
            ...
            <tr className="bg-gray-100 font-bold">
              <td className="py-3 px-4 text-gray-900 border-t border-gray-300">Grand Total</td>
              {versions.map((v) => (
                <td key={v.id} colSpan={3} className="py-3 px-3 text-right border-t border-l border-gray-300 text-gray-900">
                  ₹{fmt(versionTotal(v.id))}
                </td>
              ))}
              <td className="py-3 px-2 border-t border-l border-gray-300" />
              <td className={`py-3 px-3 text-right border-t border-gray-300 ${isOverallOverBudget ? "text-red-600" : "text-green-700"}`}>
                <div className="flex items-center justify-end gap-1">
                  {isOverallOverBudget && <FiAlertTriangle className="w-4 h-4" />}
                  ₹{fmt(totalApprovedSpend)}
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div className="mt-4 flex items-center gap-4 text-xs text-gray-500 flex-wrap">
        ...
        <div className="flex items-center gap-1.5">
          <FiAlertTriangle className="w-3.5 h-3.5 text-red-500" />
          <span>Approved spend exceeds latest planned total</span>
        </div>
      </div>
    </div>
  );
