...
import { FiPlus, FiX, FiChevronDown, FiUpload, FiBell, FiMail, FiMessageCircle } from "react-icons/fi";
import { CATEGORIES, useEventCosting } from "../Eventcostingcontext";
...
const inputCls =
  "w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white";
const numCls =
  "px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";

// Only "Approved" gets green — everything else stays neutral.
const STATUS_STYLES = {
  Approved: "bg-green-100 text-green-700",
...
function SendReminderMenu({ onSend }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-2 px-4 py-2 border border-gray-300 text-blue-600 rounded-lg hover:bg-gray-50 transition-colors text-sm"
      >
        <FiBell className="w-4 h-4" />
        Send Reminder
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-1 w-44 bg-white border border-gray-200 rounded-lg shadow-lg z-20 overflow-hidden">
            ...
            >
              <FiMail className="w-4 h-4 text-gray-400" /> Send by Email
            </button>
            <button
              onClick={() => choose("WhatsApp")}
              className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors text-left border-t border-gray-100"
            >
              <FiMessageCircle className="w-4 h-4 text-gray-400" /> Send by WhatsApp
            </button>
          </div>
        </>
      )}
...
export default function ExpenseItems() {
  ...
  const submitAddExpense = () => {
    if (!selectedCategory) return;
    handleAddExpense(selectedCategory);
    setSelectedCategory("");
  };

  const sendReminder = (expense, channel) => {
    // Wire this up to your actual notification service.
    console.log(`Reminder for "${expense.category}" (${expense.vendorName || "no vendor"}) via ${channel}`);
  };
    ...
    <div className="mt-8 pt-6 border-t border-gray-200 bg-white py-6">
      ...
      <div className="space-y-4">
        {expenses.map((expense) => {
          const totalAmount = expense.subItems.reduce((s, si) => s + (Number(si.amount) || 0), 0);
          const isApproved = expense.approvalStatus === "Approved";
          return (
            <div
              key={expense.id}
              className={`rounded-xl p-6 border shadow-sm transition-colors border-gray-200
                `}>
                {/* ${isApproved ? "bg-green-50 border-green-200" : "bg-white "} */}

              {/* Header */}
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-base font-bold text-gray-900">{expense.category}</h3>
                <button onClick={() => handleRemoveExpense(expense.id)} className="text-gray-400 hover:text-red-500 transition-colors">
                  <FiX className="w-5 h-5" />
                </button>
              </div>
              ...
              {/* Row 3: Total Amount (derived from sub-items) + Remark */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                <Field label="Total Amount">
                  <div className={`${inputCls} bg-gray-100 text-gray-700 font-medium`}>{fmtAmt(totalAmount)}</div>
                </Field>
                <Field label="Remark">
                  <input
                    type="text"
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => handleExpenseChange(expense.id, "approvalStatus", e.target.value)}
                    className={`px-2.5 py-1 rounded-full text-xs font-medium border-0 focus:outline-none focus:ring-2 focus:ring-blue-400 ${STATUS_STYLES[expense.approvalStatus]}`}
                  >
                ...
                <div className="flex gap-2">
                  <SendReminderMenu onSend={(channel) => sendReminder(expense, channel)} />
                  <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium">
                    Send for approval
                  </button>
                </div>
