...
import { FiX, FiTrash2, FiChevronDown, FiCheckCircle, FiUpload } from "react-icons/fi";
...
function rowsToContacts(rows) {
  return rows
    .map((row) => {
      ...
      return { name: name.trim(), phone: phone.trim(), email: email.trim(), group, hasWhatsApp: !!phone.trim() };
    })
    .filter((c) => c.name);
}

export default function ContactsModal({ mode, contacts, groups, onClose, onAddContact, onImportContacts, onDeleteContact }) {
  ...
  const handleCsvChange = (e) => {
    ...
    reader.onload = () => {
      ...
      const newContacts = rowsToContacts(rows);
      if (newContacts.length === 0) {
        setCsvNotice("No valid rows found. Expect columns: name, phone, email, group.");
      } else {
        onImportContacts(newContacts);
        setCsvNotice(`Imported ${newContacts.length} contact${newContacts.length === 1 ? "" : "s"}.`);
      }
    };
            ...
            <div className="bg-[#eff6ff] rounded-[12px] p-4 mb-5">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-5 h-[42px] rounded-[8px] border border-[#d1d5db] bg-white text-[13px] font-semibold text-[#333] hover:bg-[#f9fafb] cursor-pointer transition-colors"
                >
              ...
              <p className="text-[12px] text-[#9ca3af] mt-2">CSV columns: name, phone, email, group.</p>
